...
def dbc_to_h(dbc_file, h_file):
    ...
    for message in db.messages:
        ...
        for signal in message.signals:
            ...
            signalType = "Signed" if signal.is_signed == "Signed" else "Unsigned"
            signalString = "Make" + byteOrder + signalType + "CANSignal(dataType," + str(signal.start) + "," + str(signal.length) + "," + str(signal.scale) + "," + str(signal.offset) + ") " + signal.name + "_Signal{};\n"
        ...
        tx_message = "CANTXMessage<" + str(len(signals)) + "> " + message.name + "_TX_Message{can_bus, 0x" + format(message.frame_id, 'x') + ", DLC, Freq, timer_group, " + ', '.join(signals) + "};\n"
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python dbc_to_csv.py <input_dbc_file> <output_h_file>")
        sys.exit(1)
